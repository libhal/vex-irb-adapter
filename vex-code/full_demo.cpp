...
public:
  ...
  // : m_scan_thread(sampling_thread, this)
  {
  }
...
private:
  ...
  static request_response<ArraySize> request_data(FILE* p_port, char p_command)
  {
    ...
    for (int attempts = 0; attempts < 10 and iterator != end; attempts++) {
      vex::wait(1, seconds); // KAMMCE FIX THIS!
    ...
    if (iterator != end) {
      ...
      printf("Failed read from port! %zu bytes read. Contents: [",
             bytes_remaining);
